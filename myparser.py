class SPParser:
    ...
    def collect_licensing_debts(self):
        ...
        debts = self.get_debts_from_json('Licenciamento')
        collection = []
        if debts is not None:
            to_collection = {
                'amount': float(debts.get('TaxaLicenciamento'))/100,
                'description': debts.get(
                    'DescricaoServico',
                    f"Licenciamento {debts['Exercicio']}"
                ),
                'title': 'Licenciamento',
                'type': 'licensing',
                'year': debts.get('Exercicio'),
            }
            collection.append(to_collection)
        return collection
    ...
    def collect_all_debts(self):
        collection = []
        collection.append(self.collect_ipva_debts())
        collection.append(self.collect_ticket_debts())
        collection.append(self.collect_insurance_debts())
        collection.append(self.collect_licensing_debts())
        return collection
